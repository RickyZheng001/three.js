 ...
 */
var g_xianShiJiShuYaoQiu = [];
var suiJiNum = null;
var yiJiMing = null;
var m_daoJuSearchArray = [];
var checked = {};
var Dc = null;
var daoju = null;
var qieXiao = null;
...
var g_resultJsonObj = {};
var img = null;
var g_generateServerIDBtn = null;
var g_zTreeMap = {};
...
function AddParamToParamArray(param)
{
    if(g_paramArray == null || g_paramArray == undefined)
    {
        return;
    }

    var same = false;
    if (g_paramArray.length == 0)
    {
    ...
    else
    {
        ...
        {
            if (param.Name == g_paramArray[i].Name)
            {
                g_paramArray[i] = param;
                same = true;
                break;
            }
        }
...
function UpdateGUIByJsonObj(guiRoot,jsonObj,onComboBoxChangedCallBack)
{
    ...
    for (var j = 0; j < paramList.length; j++) {
        ...
        if (paramType == "EditBox") {
            ...
            if (paramName == "Dc")
            {
                Dc = param[paramName] = defaultValue;
            }
            var bFind = false;
            var index = 0;
        ...
        else if (paramType == "ComboBox")
        {
            ...
            if(typeof(str) == "string") {
                ...
                if(bFind == false)
                {
                    guiRoot.add(param, paramName, selections).onFinishChange(onComboBoxChangedCallBack).listen();
                    if(param.OnChange)
                    {
                        ctrl.OnChange = param.OnChange;
                        ctrl.ParamObj = param;
        ...
        else if(paramType == "GenerateServerID")
        {
            ...
            if(bFind == false)
            {
                ...
                g_generateServerIDBtn = guiRoot.add(param, paramName);
            }
...
function LoadLeftPanelUIConfig(configFileURL)
{
    var scope = this;
...
function LoadZuanTouSearchGUI(configFileURL)
{
    var scope = this;
    var loader = new THREE.FileLoader( scope.manager );
    loader.load( configFileURL, function ( text ) {
        ...
        m_Search = jsonObj;
        UpdateParamDesignByJson_NobelTechss(jsonObj);
    });
}
function ZuanTouSerach() {
    var tiaoJianList = [];
    var tiaoJianSearch = [];
    var sql = "";
    var ulList = document.getElementById("daoJuSearch_main_left_bottom");
...
function LoadDesignGUI(configFileURL,callback,menuObj,onClickUpdateCallBack,onComboBoxChangedCallBack,onSearch)
{
    ...
    loader.load( configFileURL, function ( text ) {
        ...
        menuObj["更新模型"] = function () {
            ...
            g_xianShiJiShuYaoQiu = tempJsonObj;
            ...
            tempJsonObj.Key = urlParam.Key;
            tempJsonObj.Random = urlParam.Random;
                ...
                if(confirm(strTips))
                {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            UpdateButton(g_generateServerIDBtn,m_globalGui);
                        },
                        error: function (errs) {
                            error();
                        }
...
function UpdateParamList()
{
    ...
    var paramList = null;
    if(g_xianShiJiShuYaoQiu != null || g_xianShiJiShuYaoQiu != undefined)
    {
       g_xianShiJiShuYaoQiu;
       var a
    }
    ...
    paramList.
        strHtml += "<div style=\"width:80%\">" + "paramList[i]: " + strwenjianming + "</div>";
...
function UpdateParamDesignByJson_NobelTechss(jsonObj,bSubMenu)
{
    var slotContainer = document.getElementById("daoJuSearch_main_left_bottom");
...
function OnSearchFinished(strJson)
{
    ...
    {
        img = jsonObj[0].Name;
        m_searchArray = jsonObj[0].List;
        jsonObj = jsonObj[0].List;
    }
    if(jsonObj.length > 0)
    {
        var strTableHtml = "<table id=\"table\"  cellspacing=\"0\" >";
        strTableHtml += "<tr class=\"table_tr\">";
        strTableHtml += "<td class=\"th\" style=\"width: 60px;\"></td>";
        for (var i = 0; i < (jsonObj[0].ParamList.length-1); i++)
        {
            strTableHtml += "<td class=\"th\" style=\"padding: 0px 3px\">" + jsonObj[0].ParamList[i].Name + "</td>";
        }
        strTableHtml += "</tr>";
        for(var j = 0; j < jsonObj.length; j++)
        {
            ...
            strTableHtml += "<td class=\"th_hidden\" style=\"width: 40px;\"><input type=\"radio\" name=\"radio\"  value=\"" + j +"\"" + string +"/></td>";
            for (var k = 0; k < (paramList.length-1); k++ )
            {
                strTableHtml += "<td class=\"th_hidden\" style=\"padding: 0px 3px\">" + paramList[k].Value + "</td>";
            }
    ...
    {
        var strTableHtml = "<table class=\"table_tr\" style=\"width: 100%\" border=\"0\" >";
        strTableHtml += "<tr class=\"table_tr\">";
        strTableHtml += "<td class=\"th\" style=\"width: 100%\">对不起，没有匹配到你所输入的参数</td>";
        strTableHtml += "</tr>";
    }
    strTableHtml += "</table>";
...
function liuYan() {
    var missage_bottom = document.getElementById("missage_bottom");
    suiJi(1,3);
    var url = "config/Files/CompanyName/PRAMET/liuyanjianyipingmianguanggao/" + yiJiMing + "/" + suiJiNum + ".png";
    missage_bottom.innerHTML = "<center><img src=\"" + url + "\" height=\"500\" width=\"375\"/></center>";
}
function jiShuVideo() {
    var divPdfViews = document.getElementById("divPdfViews");
    suiJi(1,4);
    var url = "config/Files/CompanyName/PRAMET/jishuziliaopdf/" + yiJiMing + "/" + suiJiNum + ".pdf";
    divPdfViews.innerHTML = "<embed id=\"pdf\" src=\"" + url + "\" type=\"application/pdf\" width=\"1440px\" height=\"670\"></embed>";
}
function peiXunVideo() {

    var video = document.getElementById("jiShuVideo_bottom");
    suiJi(1,4);
    var url = "config/Files/CompanyName/PRAMET/peixunshipin/" + yiJiMing + "/" + suiJiNum + ".mp4";
    video.innerHTML = "<video id=\"videos\" width=\"640px\" controls height=\"360px\"><source src=\"" + url + "\" type=\"video/mp4\"></video>";
    var videos = document.getElementById("videos");
    videos.play();
}
function souSuoImg() {
    var imgHtml = document.getElementById("daoJuSearch_main_right_bottom_main");
    suiJi(1,3);
    var url = "config/Files/CompanyName/PRAMET/sousuopingmianguanggao/" + yiJiMing + "/" + suiJiNum + ".png";
    imgHtml.innerHTML = "<img src=\"" + url + "\">";
}
function OnClose() {
    var Search = document.getElementById("gameCanvass");
...
function UpdateZTreeByJson_TimeLine(jsonObj)
{
    var setting = {
        view: {
            ...
            addDiyDom: this.OnAddDiyDom_NobelTech
        },
        data: {
            simpleData: {
                enable: true
...
function OnAddDiyDom_NobelTech(treeId, treeNode)
{
    var IDMark_A = "_a";
    ...
    if (treeNode)
    {
        ...
        */
        ...
        var editStr = "<span id='" + uuid + "' class='demoIcon' title='" + treeNode.name + "' onfocus='this.blur();' onclick='OnClickDiyBtn_NobelTech(this)'><span class='button icon02'></span></span>";
        aObj.append(editStr);

        g_zTreeMap[uuid] = treeNode;

        // /var btn = $("#diyBtn_" + treeNode.id);
        //if (btn) btn.bind("click", function(){alert("diy Button for " + treeNode.name);});
    }
}
function OnClickDiyBtn_NobelTech(btn)
{
    var strId = btn.id;

    var treeNode = g_zTreeMap[strId];
...
function ReloadTimeLineByZTree_NobelTech()
{
    ...
    for ( var i = 0; i < childNodes.length; i++)
    {
        ...
        <li style="width: 253px; float: left; display: block;">
            ...
            strHtml += "<div class='smallbutton' style='width:80px;position: absolute;left: 80px;top: 222px;z-index: 1' onclick='OnClickLuoXuanCaoZuanTouTimeLine_WaiXingSheJi(0)'>进入</div>";
            strHtml += "<div class='item'>";
            strHtml += "<h3>" + childNodes[i].name + "</h3>";
            strHtml += "<div class='desc'>";
            strHtml += "<p>" + childNodes[i].Desc + "" + "</p>";
            strHtml += "</div>";
            strHtml += "</div>";
            strHtml += "</li>";
        }
...
function XianSuDu(value) {
    ...
    if (g_daoJuIniString != null)
    {
        for (var i = 0; i < g_daoJuIniString.Tab.length; i++)
        {
            for (var j = 0; j < g_daoJuIniString.Tab[i].ParamList.length; j++)
            {
                if (g_daoJuIniString.Tab[i].ParamList[j].Name == "Dc")
                {
                    Dc = g_daoJuIniString.Tab[i].ParamList[j].Value;
                }
            }
        }
    }
    for (var i = 0; i < g_paramArray.length; i++) {
        if (g_paramArray[i].Name == "转速 rpm") {
            g_paramArray[i][g_paramArray[i].Name] = Math.round(value * 1000 / (3.14 * Dc));
            break;
        }

    }
}
function XianSuDu1(value) {
    var D = null;
    for (var i = 0; i < g_paramArray.length; i++) {
        if (g_paramArray[i].Name == "零件直径") {
            D = g_paramArray[i][g_paramArray[i].Name];
            break;
        }
    }
    for (var i = 0; i < g_paramArray.length; i++) {
        if (g_paramArray[i].Name == "转速 rpm") {
            g_paramArray[i][g_paramArray[i].Name] = Math.round(value * 1000 / (3.14 * D));
            break;
        }

    }
}
function ZhiJing(value) {
    var v = null;
    for (var i = 0; i < g_paramArray.length; i++) {
        if (g_paramArray[i].Name == "线速度 m/min") {
            v = g_paramArray[i][g_paramArray[i].Name];
            break;
        }
    }
    for (var i = 0; i < g_paramArray.length; i++) {
        if (g_paramArray[i].Name == "转速 rpm") {
            g_paramArray[i][g_paramArray[i].Name] = Math.round(v * 1000 / (3.14 * value));
            break;
        }
...
function MeiChiJinJiLiang(value)
{
    ...
    for(var i = 0; i < g_paramArray.length; i++)
    {
        if(g_paramArray[i].Name == "转速 rpm")
        {
            n = g_paramArray[i][g_paramArray[i].Name];
            break;
        }
    }
    for(var i = 0; i < g_paramArray.length; i++)
    {
        ...
        {
            g_paramArray[i][g_paramArray[i].Name] = value * n;
            break;
        }
    }

}
function JiaGongChangDu(value)
{
    var F = null;
    for(var i = 0; i < g_paramArray.length; i++)
    {
        if(g_paramArray[i].Name == "每分钟进给量 mm/min")
        {
            F = g_paramArray[i][g_paramArray[i].Name];
            break;
        }
    }
    for(var i = 0; i < g_paramArray.length; i++)
    {
        if(g_paramArray[i].Name == "加工时间 sec")
        {
            g_paramArray[i][g_paramArray[i].Name] = value/F;
            break;
        }
    }
}
...
function onSearchZuanTouCallBacks(strJson) {
    m_daoJuSearchArray = {};
    var searchDiv = document.getElementById("daoJuSearch_main_right_top_bottom");
    var jsonObj = JSON.parse(strJson);
    if (jsonObj.length > 0)
    {
        img = jsonObj[0].Name;
        m_daoJuSearchArray= jsonObj[0].List;
        jsonObj = jsonObj[0].List;
    }
    ...
    {
        var strTableHtml = "<table id=\"table\" cellspacing=\"0\">";
        strTableHtml += "<tr class=\"tabTh table_tr\">";
        strTableHtml += "<td class=\"th\" style=\"width: 40px;\"></td>";
        for (var i = 0; i < (jsonObj[0].ParamList.length-1); i++)
        {
            strTableHtml += "<td class=\"th\" style=\"padding: 3px\">" + jsonObj[0].ParamList[i].Name + "</td>";
        }
        strTableHtml += "</tr>";
        for(var j = 0; j < jsonObj.length; j++)
        {
            ...
            var string = (j == 0 ? "checked" : "unchecked");
            strTableHtml += "<tr class=\"table_tr\"  onclick=\"xuanzhong(this)\">";
            strTableHtml += "<td class=\"th_hidden\" style=\"width: 40px;\"><input type=\"radio\" name=\"ra\"  value=\"" + j +"\"" + string +"/></td>";
            for (var k = 0; k < (paramList.length-1); k++ )
            {
                strTableHtml += "<td class=\"th_hidden\">" + paramList[k].Value + "</td>";
            }
            strTableHtml += "</tr>";
        }
        strTableHtml += "</table>";
        var imgHtml = document.getElementById("daoJuSearch_main_right_bottom_main");
        var imgcolor = document.getElementById("daoJuSearch_main_right_bottom");
        imgHtml.innerHTML = "<img src=\"images/helpImages/"+ img +"2.png\">";
        imgcolor.style.backgroundColor = "#FFF3D5";
    ...
    else
    {
        var strTableHtml = "<table class=\"table_tr\" style=\"width: 100%\" border=\"0\">";
        strTableHtml += "<tr class=\"table_tr\">";
        strTableHtml += "<td class=\"th\" style=\"width: 100%\">空</td>";
        strTableHtml += "</tr>";
        strTableHtml += "</table>";
        var imgHtml = document.getElementById("daoJuSearch_main_right_bottom_main");
        imgHtml.innerHTML = "<img src=\"images/underfind.png\">";
    }
    searchDiv.innerHTML = strTableHtml;
...
function suiJi(min,max){
    ...
    suiJiNum = min + Math.round(Rand*Range);
}
function videoVisible() {
    suiJi(1,4);
    var url = "config/Files/CompanyName/PRAMET/gengxinshipin/" + yiJiMing + "/" + suiJiNum + ".mp4";
    var html = document.getElementById("progress_bottom");
    var progress = document.getElementById("progress");
    var zi = document.getElementById("zi");
    var xx = document.getElementById("xx");
    var img = document.getElementById("porgress_main_top");
    xx.style.visibility = "hidden";
    zi.innerText = "正在生成，请不要关闭，耐心等待......";
    img.innerHTML = "<img src=\"images/loading.gif\" height=\"140\" width=\"224\">";

    progress.style.width = "590px";
    progress.style.height = "675px";
    html.innerHTML = "<video id=\"video\" style=\"margin-top: 50px\" width=\"550px\" height=\"310px\" loop=\"loop\"><source src=\"" +url + "\" type=\"video/mp4\"></video>";
    startVideo();
}
function imgVisible() {
    suiJi(1,4);
    var url = "config/Files/CompanyName/PRAMET/zhuangpeipingmianguanggao/" + yiJiMing + "/" + suiJiNum + ".png";
    var html = document.getElementById("progress_bottom");
    var progress = document.getElementById("progress");
    var zi = document.getElementById("zi");
    var img = document.getElementById("porgress_main_top");
    ...
    progress.style.height = "740px";
    html.innerHTML = "<center><img src=\"" + url + "\" height=\"500\" width=\"375\"/></center>";
}
...
function sss() {
    ...
    var xx = document.getElementById("xx");
    var img = document.getElementById("porgress_main_top");
    zi.innerText = "模型已生成,5秒后自动关闭本窗口\r\n如没有关闭请手动关闭";
    xx.style.visibility = "visible";
    img.innerHTML = "<img src=\"images/sss.gif\" height=\"140\" width=\"224\">";
...
function error() {
    ...
    var zi = document.getElementById("zi");
    var xx = document.getElementById("xx");
    zi.innerText = "模型生成失败，请重新生成";
    xx.style.visibility = "visible";
    img.innerHTML = "<img src=\"images/error.gif\" height=\"140\" width=\"224\">";
}
function UpdateButton(guiRoot,m_globalGui) {
    if (guiRoot.domElement == null || guiRoot.domElement == undefined)
    {
    }else
    {
        guiRoot.domElement.innerText = g_serverGenerateID;
        m_globalGui.__controllers[0].domElement.innerText = g_serverGenerateID;
    }

}
function UpdateButtons(guiRoot,m_globalGui,id) {

    guiRoot.domElement.innerText = id;
    m_globalGui.__controllers[0].domElement.innerText = id;
}
function OnChecks(check)
{
    var id = null;
    if (m_daoJuSearchArray.length < 1)
    {
        alert("请搜索内容");
        return;
    }
    if(check == 1)
    {
        var r = confirm("继承所选中的参数将传至DESIGN？");
    } else if (check == 2)
    {
        var r = confirm("修改所选中的参数将传至DESIGN？");
    }
    ...
    if (r == true)
    {
        ...
        for (i=0; i<radio.length; i++)
        {
            if (radio[i].checked)
            {
                checked = m_daoJuSearchArray[i].ParamList;
                break;
            }
        }
        ...
        if (check == 1)
        {
            ...
            {
                for (var k = 0; k < g_paramArray.length; k++)
                {

                    if ( g_paramArray[k].iniName == checked[i].Name && g_paramArray[k].iniName != "FN"/* && g_paramArray[k].iniName == "dm"*/)
                    {
                        if (g_paramArray[k].Type == "EditBox")
                        {
                            g_paramArray[k][g_paramArray[k].Name] = "" + checked[i].Value;
                        }
                        else if (g_paramArray[k].Type == "ComboBox")
                        {
                            var defaultIndex = null;
                            if (g_paramArray[k].Choise.split == undefined || g_paramArray[k].Choise.split == null)
                            {
                                for (var j = 0; j < g_paramArray[k].Choise.length; j++)
                                {
                                    if (g_paramArray[k].Choise[j].Name == checked[i].Value)
                                    {
                                        defaultIndex = j;
                                        break;
                                    }
                                ...
                                var strArray = g_paramArray[k].Choise.split(","); //字符分割

                                for(var j = 0; j < strArray.length; j++)
                                {
                                    if (strArray[j] == checked[i].Value)
                                    {
                                        defaultIndex = j;
                                        break;
                                    }
                            ...
                            g_paramArray[k][g_paramArray[k].Name] = defaultIndex;
                        }
        ...
        } else if (check == 2)
        {
            for (var i = 0; i < checked.length; i++)
            {
                for (var k = 0; k < g_paramArray.length; k++)
                {
                    if (g_paramArray[k].iniName == checked[i].Name)
                    {
                        if (g_paramArray[k].Type == "EditBox")
                        {
                            g_paramArray[k][g_paramArray[k].Name] = "" + checked[i].Value;
            ...
            for (var i = 0; i < checked.length; i++)
                {
                    if (checked[i].Name == "FN")
                    {
                        g_serverGenerateID = "" + checked[i].Value;
                    }
                }
                UpdateButtons(g_generateServerIDBtn,m_globalGui,g_serverGenerateID);

        }
        OnClickZuanTouSearch();
    }
}
